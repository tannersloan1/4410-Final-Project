...
$teachers_r = $conn->query("SELECT ti.teacher_id, ti.email, ti.full_name, tu.created_at FROM TEACHER_INFO ti LEFT JOIN TEACHER_USERS tu ON ti.teacher_id=tu.teacher_id ORDER BY ti.teacher_id DESC");
?>
...
.field input{width:100%;padding:10px 13px;background:#0f172a;border:1px solid #334155;border-radius:8px;color:#f1f5f9;font-size:.92rem;font-family:inherit;transition:border-color .2s;box-sizing:border-box;}
.field input:focus{outline:none;border-color:#3b82f6;}.field input::placeholder{color:#475569;}
...
.btn-del{padding:5px 11px;background:#7f1d1d;color:#fca5a5;border:none;border-radius:6px;font-size:.78rem;font-weight:700;cursor:pointer;text-decoration:none;display:inline-block;}
...
<main class="page">
<div class="breadcrumb"><a href="admin-dash.php">Dashboard</a> › Teachers</div>
<div class="hero"><h1>✏️ Manage Teachers</h1><p>Create teacher accounts and view who's on the platform.</p></div>
...
<div class="two-col">
    <div class="card">
        ...
        <form method="POST">
            <div class="field"><label>Full name</label><input type="text" name="full_name" placeholder="Jane Smith" required></div>
            <div class="field"><label>Email</label><input type="email" name="register-email" placeholder="teacher@school.edu" required></div>
            <div class="field"><label>Temporary password</label><input type="password" name="password" placeholder="Min 6 characters" required></div>
    ...
    <div class="card">
        ...
        <table class="t-table">
            ...
            <tr>
                ...
                <td><a href="teacher.php?delete=<?= $t["teacher_id"] ?>" class="btn-del" onclick="return confirm('Delete this teacher? This cannot be undone.')">Delete</a></td>
            </tr>
            <?php endwhile; ?>
            </tbody>
        </table>
        <?php else: ?><p style="color:#475569">No teachers yet.</p><?php endif; ?>
    </div>
</div>
</main>
